...
def birdclef2024score(y_pred, y_true):
    ...
    y_true_bin=np.array([np.argmax(y) for y in y_true])
    m = nn.Softmax(dim=1)
    yy = torch.from_numpy(np.array(y_pred).astype(np.float32))
    y_pred_ = m(yy).detach().numpy().copy()

    print(y_pred_.shape)
    print(len(y_true_bin))

    return roc_auc_score( 
        y_true_bin, 
        y_pred_,
        multi_class='ovr',
        labels=[i for i in range(y_pred_.shape[1])]
        )

    pred_df = pd.DataFrame(y_pred, columns=label_list)
    pred_df['id'] = np.arange(len(pred_df))
    gt_df = pd.DataFrame(y_true, columns=label_list)
